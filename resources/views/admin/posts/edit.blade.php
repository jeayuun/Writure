...
@section('content')
    <form action="{{ route('posts.update', $post->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-semibold font-serif text-gray-800">{{ __('Edit Post') }}</h1>
            <div class="flex items-center">
                ...
                <button type="submit" class="bg-black text-white px-5 py-2 rounded-md font-normal hover:bg-gray-800 transition">
                    <i class="fas fa-save mr-2"></i> {{ __('Update Post') }}
                </button>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2">
                <div class="bg-white p-8 rounded-lg shadow-sm">
                    <div x-data="{ activeTab: '{{ $language->slug ?? $languages->first()->slug }}' }">
                        <div class="border-b border-gray-200">
                            <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                                @foreach ($languages as $lang)
                                    <a href="#" @click.prevent="activeTab = '{{ $lang->slug }}'"
                                       :class="{
                                            'border-black text-gray-900': activeTab === '{{ $lang->slug }}',
                                            'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300': activeTab !== '{{ $lang->slug }}'
                                       }"
                        ...
                        <div class="mt-6">
                            @foreach ($languages as $lang)
                                ...
                                <div x-show="activeTab === '{{ $lang->slug }}'" x-cloak class="space-y-6">
                                    <input type="hidden" name="translations[{{ $lang->slug }}][language_slug]" value="{{ $lang->slug }}">
                                    {{-- Title --}}
                                    <div>
                                        <label for="title_{{ $lang->slug }}" class="block text-sm font-medium text-gray-700">{{ __('Title') }}</label>
                                        <input type="text" name="translations[{{ $lang->slug }}][title]" id="title_{{ $lang->slug }}" value="{{ old('translations.'.$lang->slug.'.title', $translation->title ?? '') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-black focus:ring-black sm:text-sm" placeholder="{{ __('Enter title') }}" {{ $lang->is_default ? 'required' : '' }}>
                                    </div>
                                    {{-- Slug --}}
                                    <div>
                                        <label for="slug_{{ $lang->slug }}" class="block text-sm font-medium text-gray-700">{{ __('Slug') }}</label>
                                        <input type="text" name="translations[{{ $lang->slug }}][slug]" id="slug_{{ $lang->slug }}" value="{{ old('translations.'.$lang->slug.'.slug', $translation->slug ?? '') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-black focus:ring-black sm:text-sm" placeholder="{{ __('seo-friendly-url') }}" {{ $lang->is_default ? 'required' : '' }}>
                                    </div>
                                    {{-- Short Description --}}
                                    <div>
                                        <label for="short_description_{{ $lang->slug }}" class="block text-sm font-medium text-gray-700">{{ __('Short Description') }}</label>
                                        <textarea name="translations[{{ $lang->slug }}][short_description]" id="short_description_{{ $lang->slug }}" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-black focus:ring-black sm:text-sm" placeholder="{{ __('Summary of the post') }}">{{ old('translations.'.$lang->slug.'.short_description', $translation->short_description ?? '') }}</textarea>
                                    </div>
                                    {{-- Content --}}
                                    <div>
                                        <label for="content_{{ $lang->slug }}" class="block text-sm font-medium text-gray-700">{{ __('Content') }}</label>
                                        <textarea name="translations[{{ $lang->slug }}][content]" id="content_{{ $lang->slug }}" class="summernote">{{ old('translations.'.$lang->slug.'.content', $translation->content ?? '') }}</textarea>
                                    </div>
                                </div>
                            @endforeach
                        </div>
            ...
            <div class="space-y-8">
                ...
                <div class="bg-white p-8 rounded-lg shadow-sm">
                     ...
                     <div class="space-y-2">
                        ...
                        <img src="{{ $post->cover_image ? asset($post->cover_image) : '' }}" class="mt-4 h-48 w-full object-cover rounded-lg shadow {{ !$post->cover_image ? 'hidden' : '' }}" id="cover-preview">
                    </div>
                </div>
                        ...
                        <div class="flex flex-wrap gap-2 border rounded-lg p-2">
                            @foreach ($tags as $tag)
                                <label class="flex items-center space-x-2 bg-gray-100 px-3 py-1 rounded-full hover:bg-gray-200 transition cursor-pointer">
                                    <input type="checkbox" name="tags[]" value="{{ $tag->id }}" class="rounded text-black focus:ring-black h-4 w-4" {{ $post->tags->contains($tag->id) ? 'checked' : '' }}>
    ...
    <script>
        $(document).ready(function() {
            $('.summernote').each(function() {
                ...
                if ('{{ $language->slug ?? $languages->first()->slug }}' === lang) {
                     $(this).summernote({
                        height: 300,
                        toolbar: [
                            ['style', ['style']],
